...
function BT(a){
    ...
    if(a.length==4){
    num=parseInt(a[2]+a[3]);    
    }
    for(i=0;i<COL.length;i++){
        if(num==COL[i]||(num+num%2*2-1)==COL[i]){}
        else{
        if(document.getElementById(a[0]+a[1]+COL[i])!= null){
        document.getElementById(a[0]+a[1]+COL[i]).style.display="none";}}
    }
    var com=a;
    if( document.getElementById(com).style.display=="block"){
    document.getElementById(com).style.display="none";
